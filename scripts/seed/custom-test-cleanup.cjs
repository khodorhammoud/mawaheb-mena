...
async function cleanupCustomTestData() {
  ...
  // Parse the ID to a number, with a fallback
  const userId = testUserId ? parseInt(testUserId, 10) : 99999;
  console.log(`Using test user ID for cleanup: ${userId} (type: ${typeof userId})`);
  ...
  try {
    ...
    await pool.query('DELETE FROM job_applications WHERE id = $1', [userId + 3000]);

    // 2. Delete freelancer skills
    console.log('Removing test freelancer skills...');
    await pool.query('DELETE FROM freelancer_skills WHERE freelancer_id = $1', [userId + 2000]);

    // 3. Delete test freelancer profile
    console.log('Removing test freelancer profile...');
    await pool.query('DELETE FROM freelancers WHERE id = $1', [userId + 2000]);
