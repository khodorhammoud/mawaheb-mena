...
const slides: RegistrationSlideData[] = [
    { image: 'https://img.freepik.com/premium-photo/young-adult-arabian-sales-agent-formal-attire-holding-laptop-slightly-smiling-modern_868783-106989.jpg?semt=ais_hybrid', quote: 'Mawaheb MENA has been a game-changer for our company. The platform provided us with access to a pool of highly skilled freelancers.', name: 'Ahmad Ramal', title: 'CEO, Waxy', rating: 4 },
    { image: 'https://img.freepik.com/premium-photo/young-adult-arabian-sales-agent-formal-attire-holding-laptop-slightly-smiling-modern_868783-106989.jpg?semt=ais_hybrid', quote: 'Finding the right talent has never been easier. Mawaheb connected us with professionals who truly understand our needs.', name: 'Nadia Hassan', title: 'VP Engineering, BuildCo', rating: 5 },
];

export default function SignupEmployer() {
    const form = useForm({ email: '', first_name: '', last_name: '', password: '', password_confirmation: '', accountType: 'employer' });
    ...
    const submit = (e: React.FormEvent) => { e.preventDefault(); form.post('/signup-employer'); };
                        ...
                        <div className="flex flex-col items-center w-full max-w-2xl mx-auto bg-white pl-2 pr-12 mt-20">
                            ...
                            <form onSubmit={submit} className="w-full space-y-6">
                                ...
                                <div className="flex space-x-4">
                                    <div className="w-1/2"><label className="block text-sm font-medium text-gray-700 mb-1">First Name</label><input type="text" value={form.data.first_name} onChange={(e) => form.setData('first_name', e.target.value)} className="w-full border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:border-[#27638a]" /></div>
                                    <div className="w-1/2"><label className="block text-sm font-medium text-gray-700 mb-1">Last Name</label><input type="text" value={form.data.last_name} onChange={(e) => form.setData('last_name', e.target.value)} className="w-full border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:border-[#27638a]" /></div>
                                </div>
                                <div className="space-y-2">
                                    ...
                                    {form.data.password.length > 0 && (
                                        <div className="space-y-2">
                                            <div className="flex items-center space-x-2"><div className="flex-1 bg-gray-200 rounded-full h-2"><div className={`h-2 rounded-full transition-all duration-300 ${strengthColors[passwordStrength]}`} style={{ width: `${((passwordStrength + 1) / 5) * 100}%` }} /></div><span className="text-sm font-medium text-gray-600">{strengthTexts[passwordStrength]}</span></div>
                                ...
                                <div className="flex flex-col space-y-3">
                                    <div className="flex items-center space-x-3 ml-3"><input type="checkbox" required className="w-4 h-4 rounded border-gray-300" /><label className="text-sm tracking-tight text-gray-500">I accept the <a href="#" className="text-[#27638a] font-semibold hover:underline">terms and conditions</a></label></div>
                            ...
                            <div className="text-center mt-8"><p className="text-sm text-gray-600">Already have an account? <Link href="/login-employer" className="text-[#27638a] font-medium hover:underline">Login</Link></p></div>
                        </div>
                    </div>
