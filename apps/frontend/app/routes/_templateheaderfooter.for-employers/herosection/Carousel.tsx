...
const carouselData = [
  {
    ...
    hourlyRate: '$20/hour',
    skills: [
      { name: 'Responsive design', isStarred: true },
      { name: 'HTML5', isStarred: true },
      { name: 'Node.js', isStarred: false },
      { name: 'Agile', isStarred: false },
      { name: 'Debugging', isStarred: false },
    ],
  },
  {
    image: 'https://www.fivebranches.edu/wp-content/uploads/2021/08/default-image.jpg',
    ...
    skills: [
      ...
      { name: 'CSS3', isStarred: true },
      { name: 'TypeScript', isStarred: true },
      { name: 'Testing', isStarred: false },
      { name: 'Performance', isStarred: false },
    ],
  },
  {
    image: 'https://www.fivebranches.edu/wp-content/uploads/2021/08/default-image.jpg',
  ...
  },
  {
    ...
    hourlyRate: '$20/hour',
    skills: [
      { name: 'Responsive design', isStarred: false },
      { name: 'HTML5', isStarred: false },
      { name: 'Node.js', isStarred: false },
      { name: 'Agile', isStarred: false },
      { name: 'Debugging', isStarred: false },
    ],
...
const Carousel = () => {
  ...
  const handleNext = () => {
    setCurrentIndex(prevIndex => (prevIndex < carouselData.length - 1 ? prevIndex + 1 : 0));
    setShowFingerIcon(false); // Hide the icon after the first swipe
  };

  // Function to handle the previous slide
  const handlePrev = () => {
    setCurrentIndex(prevIndex => (prevIndex > 0 ? prevIndex - 1 : carouselData.length - 1));
    setShowFingerIcon(false); // Hide the icon after the first swipe
  };
  ...
  return (
    <div className="">
      <div
        {...handlers}
        className="relative w-full h-screen mx-auto mt-10 select-none overflow-x-hidden"
      >
