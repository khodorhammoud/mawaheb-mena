...
import { GET_EMPLOYERS_SUBHEADLINE_QUERY } from "../../../../../cms/graphql/queries";
...
interface SubHeadlineResponse {
  data: {
    forEmployersSubHeadlines: SubHeadline[]; // Use the corrected plural name
  };
  errors?: GraphQLError[]; // This remains the same
}
    ...
    async function fetchSubHeadline() {
      try {
        const res = await fetch("http://localhost:3000/api/graphql", {
          ...
          body: JSON.stringify({
            query: GET_EMPLOYERS_SUBHEADLINE_QUERY, // Use the updated query
          }),
        });

        const json: SubHeadlineResponse = await res.json();
        console.log("Fetched subheadline:", json); // Log the entire response

        if (json.errors) {
          console.error("GraphQL errors:", json.errors); // Log any GraphQL errors
        }

        if (json.data && json.data.forEmployersSubHeadlines.length > 0) {
          const fetchedContent = json.data.forEmployersSubHeadlines[0].content; // Get the first subheadline
          setSubHeadlineContent(fetchedContent);
        } else {
          console.error("No subheadline found in response");
        }
      } catch (error) {
        console.error("Error fetching subheadline:", error); // Log general errors
      }
    }
