...
function GeneralizableFormCard(props: GeneralizableFormCardProps) {
  ...
  const [inputValue, setInputValue] = useState<number | string | File>(
    props.formType !== 'repeatable'
      ? (initialData?.[props.fieldName] ?? (props.formType === 'increment' ? 0 : ''))
      : null
  );
  ...
  const handleRemoveRepeatableField = (index: number) => {
    setRepeatableInputValues(
      prevValues => prevValues.filter((_, i) => i !== index) as typeof repeatableInputValues
    );
  ...
  const handleFileChange = (index: number, file: File) => {
    ...
    switch (props.repeatableFieldName) {
      case 'portfolio':
        updatedInputValues = [...repeatableInputValues] as PortfolioFormFieldType[];
        (updatedInputValues[index] as PortfolioFormFieldType).projectImageName = file.name;
        setRepeatableInputValues(updatedInputValues);
        break;
      case 'certificates':
        updatedInputValues = [...repeatableInputValues] as CertificateFormFieldType[];
        (updatedInputValues[index] as CertificateFormFieldType).attachmentName = file.name;
        setRepeatableInputValues(updatedInputValues);
        break;
      case 'educations':
        updatedInputValues = [...repeatableInputValues] as EducationFormFieldType[];
        setRepeatableInputValues(updatedInputValues);
        break;
      default:
        break;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (props.formType === 'repeatable') {
      ...
      repeatableInputFiles.forEach((file, index) => {
        formData.append(`${props.repeatableFieldName}-attachment[${index}]`, file);
      });
    }
          ...
          <div className="flex flex-col items-center space-y-4 w-full">
            <div className="flex items-center border border-gray-300 rounded-xl w-full">
              ...
              <div className="w-full h-12 flex justify-center items-center border-x border-gray-300 text-lg">
                {typeof inputValue === 'number' || typeof inputValue === 'string' ? inputValue : ''}
              </div>
                ...
                >
                  <div className="p-3">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      >
                        {expandedIndex === index ? 'Collapse' : 'Expand'} Project Form {index + 1}
                      </Button>
    ...
    <Card className="bg-gray-100 border-2 border-gray-300 rounded-xl border-dashed pl-8 pb-5 pt-5 h-auto grid w-auto">
      ...
      <Dialog>
        ...
        <DialogContent className="bg-white">
          ...
          <DialogTitle className="mt-3 tracking-normal">{props.popupTitle}</DialogTitle>
