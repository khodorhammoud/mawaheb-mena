  ...
  useEffect(() => {
    if (dialogOpen && formContentRef.current) {
      ...
      if (formRef && formRef.current && formRef.current.filesSelected) {
        formContentRef.current.setFilesSelected(formRef.current.filesSelected);
      ...
      if (value && typeof value === 'object') {
        ...
        if ('attachments' in value) {
          ...
          if (
            ...
            if (fileObjects.length > 0 && formContentRef.current.setFilesSelected) {
        ...
        if (fieldName in value && Array.isArray(value[fieldName]) && value[fieldName].length > 0) {
  ...
  return (
    <Card
      className={`border-2 rounded-xl flex flex-col h-full ${
        isFilled || hasExistingFiles ? 'bg-[#F1F0F3]' : 'bg-gray-100 border-gray-300 border-dashed'
      }`}
    >
        ...
        {editable && (
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            <DialogTrigger asChild>
              ...
              >
                {isFilled || hasExistingFiles ? triggerLabel || 'Edit' : triggerLabel || 'Add'}
              </Button>
            </DialogTrigger>
      ...
      {formContentRef.current?.filesSelected?.length > 0 && (
        <div className="text-gray-700 px-7 pb-4">
          <p className="text-sm text-gray-500">
            ...
            {new Set(formContentRef.current.filesSelected.map((f: File) => f.name)).size} file(s)
            selected
          </p>
        </div>
      )}
