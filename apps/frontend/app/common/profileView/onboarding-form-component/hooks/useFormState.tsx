...
export const useFormState = (formType: string, fieldName: string) => {
    ...
    const [repeatableInputValues, setRepeatableInputValues] = useState<RepeatableInputType[]>([]);
    ...
    const initializeRepeatableFields = (data: any, field: string) => {
        if (data?.[field]) {
            setRepeatableInputValues(JSON.parse(data[field]));
            setRepeatableInputFiles(new Array(JSON.parse(data[field]).length).fill(null));
        }
    };
    ...
    const handleAddRepeatableField = () => {
        setRepeatableInputValues(prev => [...prev, DEFAULT_FORM_FIELDS[fieldName]]);
    ...
    const handleDataChange = (index: number, updatedData: RepeatableInputType) => {
