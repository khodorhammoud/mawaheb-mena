...
export const paymentTypeEnum = pgEnum('payment_type', [
  'employer_to_platform',
  'platform_to_freelancer',
]);

// Table for storing bank account details
export const bankAccountsTable = pgTable('bank_accounts', {
  ...
  gatewayAccountId: text('gateway_account_id'), // ID from the payment gateway
...
export const paymentsTable = pgTable('payments', {
  ...
  gatewayPaymentId: text('gateway_payment_id'), // ID from the payment gateway
  sourceUserId: integer('source_user_id').references(() => UsersTable.id),
  destinationUserId: integer('destination_user_id').references(() => UsersTable.id),
  employerId: integer('employer_id').references(() => employersTable.id),
  freelancerId: integer('freelancer_id').references(() => freelancersTable.id),
  sourceBankAccountId: integer('source_bank_account_id').references(() => bankAccountsTable.id),
  destinationBankAccountId: integer('destination_bank_account_id').references(
...
// Table for admin bank accounts (platform accounts)
export const adminBankAccountsTable = pgTable('admin_bank_accounts', {
  id: serial('id').primaryKey(),
  accountHolderName: text('account_holder_name').notNull(),
  accountNumber: text('account_number').notNull(),
  iban: text('iban'),
