...
setup('authenticate as dashboard employer', async ({ page }) => {
  ...
  console.log(`Logging in with seeded dashboard employer: ${email}`);
  ...
  console.log(`After dashboard employer login: ${afterLoginUrl}`);
  if (afterLoginUrl.includes('/login')) {
    throw new Error(`Dashboard employer login failed. Still on login page: ${afterLoginUrl}`);
  }

  // 2) DB guardrails (match your schema)
  //    UsersTable.isOnboarded (boolean)
  //    accountsTable.accountStatus (enum -> stored as string like 'published')
  ...
  // 3) Go to dashboard after state changes
    ...
    JSON.stringify(
      {
        ...
        userType: 'employer',
        state: 'dashboard',
        dbState: 'Users.isOnboarded: true, Accounts.accountStatus: published',
        note: 'Guaranteed published + onboarded employer for dashboard tests. Safe to use with test.use({ storageState: dashboardEmployerFile }).',
      },
      null,
      2
    )
