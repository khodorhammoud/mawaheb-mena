...
test.describe('Employer Signup Flow', () => {
    test('should sign up a new employer successfully', async ({ page }) => {
        ...
        await expect(
            page.getByText(/A verification email has been sent to you/i)
    ...
    test('should show validation errors for missing fields', async ({ page }) => {
        ...
        await expect(page.getByText(/Email Address is required/i).first()).toBeVisible({ timeout: 260_000});
        await expect(page.getByText(/First Name is required/i).first()).toBeVisible({ timeout: 260_000});
        await expect(page.getByText(/Last Name is required/i).first()).toBeVisible({ timeout: 260_000});
    ...
    test('should show error for duplicate email', async ({ page }) => {
        ...
        await expect(page.getByText(/A verification email has been sent/i))
              .toBeVisible({ timeout: 230_000 });
        ...
        await expect(page.locator('div.bg-red-100'))
              .toHaveText(/already registered/i, { timeout: 230_000 });
      });
